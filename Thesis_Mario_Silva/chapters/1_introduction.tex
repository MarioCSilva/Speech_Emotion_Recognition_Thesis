...
An additional objective is to create a system that can perform emotional analysis both offline and in real-time, which may involve designing separate audio stream processing pipelines for each mode.
...
%Chapter \ref{chapter:strat} presents the developed emotion recognition strategies and describes their process. This chapter also evaluates different models and compares their performance.

%Based on the previously settled models, in chapter \ref{chapter:data_study} we will analyze potential data biases, such as gender, and explore the quality of the used data.

%The sixth chapter presents a \ac{ser} model applied in a video conference system, detailing its implementation and results. The next chapter \ref{chapter:fusion} details the fusion model using the audio and visual modalities, studying the significance of prioritizing one over the other in different contexts.

%The discussion chapter, discusses the accomplishment of the objectives, with remarks regarding the development process, obtained results, and suggestions for the utilized dataset(s).